...
class ExpediaPreprocessor:
    ...
    def load_and_process(self, nrows=None):
        ...
        # --- 3. Temporal Split ---
        print(
            f"[Preprocessor] Sorting and splitting by time (Ratio: {self.cfg.split_ratio})..."
        )
        ...
        self.final_feat_cols = [c for c in train_df.columns if c not in exclude_cols]

        # Identify indices of categorical features for CatBoost
        self.cat_features_indices = [
            i for i, c in enumerate(self.final_feat_cols) if c in cat_cols
        ]

        print(f"[Preprocessor] Final Feature Count: {len(self.final_feat_cols)}")
        print(
            f"[Preprocessor] Categorical Features Indices: {self.cat_features_indices}"
        )
