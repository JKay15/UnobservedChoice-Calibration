...
class SyntheticDataEngine:
            ...
            if isinstance(self.u_mapper, LinearUtilityMapper):
                ...
                if sigma > 0:
                    ...
                    if dist_type == 'gaussian':
                        ...
                        delta_beta = (torch.rand(d, device=self.cfg.device) * 2 - 1) * bound
                    else:
    ...
    def _validate_assumptions(self, z, s, eta) -> Tuple[bool, str]:
        ...
        try:
            ...
            if min_eig < check_min_eig:
                return False, f"Asm 2: Covariance singular (min_eig={min_eig:.2e} < {check_min_eig})"
        except Exception as e:
    ...
    #     # --- Model A: Structural error---
    #     if self.cfg.utility_mode == 'structural':
    #         if not isinstance(self.u_mapper, LinearUtilityMapper):
    #             raise ValueError("structural mode requires LinearUtilityMapper")
    ...
    #             if dist_type == 'gaussian':
    #                 raw = torch.randn(d, device=self.cfg.device)
    #                 # Normalize direction, scale by sigma
    #                 delta_beta = raw / (raw.norm() + 1e-9) * sigma
                    
    #             elif dist_type == 'uniform':
    #                 # total norm approx sigma.
    #                 bound = sigma / np.sqrt(d)
    #                 delta_beta = (torch.rand(d, device=self.cfg.device) * 2 - 1) * bound
    ...
    #             if dist_type == 'gaussian':
    #                 # s_hat = s + N(0, sigma^2)
    #                 # Mean Squared Error E[err^2] = sigma^2
    #                 noise = torch.randn_like(s_true) * sigma
                    
    #             elif dist_type == 'uniform':
    #                 # s_hat = s + U[-sigma, sigma]
    #                 # Max Error |err| <= sigma (Strict Bound)
    #                 noise = (torch.rand_like(s_true) * 2 - 1) * sigma
