...
def run_single_trial(cfg: ExpConfig, 
                     ...
                     norm:bool=True,
                     sim_noise_type: str = 'gaussian') -> Dict[str, float]: # norm=True for theory validation
    ...
    engine = EngineFactory.build_synthetic_engine(
        cfg, z_type=z_type, u_type=u_type, y_type=y_type, context_mapper_type=context_type,norm=norm
    )
    ...
    base_algo = algo_type.replace('_oracle', '') if task_type!='multi_sim' else multi_sim_method.replace('_oracle', '')
    # Select Input Data:
    ...
    # ==========================================
    if task_type == 'multi_sim' and (multi_sim_method in ['logit_mean', 'weighted_mean', 'median']):
        eta_true = truth['eta']
        ...
        # Sim 2: Slight Shift
        mapper2 = MonotoneYMapper(replace(cfg, sim_bias_a=cfg.sim_bias_a+0.5, sim_bias_b=cfg.sim_bias_b*0.5))
        # Sim 3: Another Shift
        mapper3 = MonotoneYMapper(replace(cfg, sim_bias_a=cfg.sim_bias_a-0.5, sim_bias_b=cfg.sim_bias_b*0.5))
        # Sim 4: High Variance
        mapper4 = MonotoneYMapper(replace(cfg, sim_bias_a=cfg.sim_bias_a+1.0, sim_bias_b=cfg.sim_bias_b*1.0))
        ...
        gamma_hat = solver.solve_multi_mrc(inputs['z'], s_input, Y_multi, method=multi_sim_method)
        ...
        if regret_need:
            ...
            if u_type != 'linear':
                ...
                if is_oracle:
                    ...
                    if beta_hat is None: beta_hat = beta_true
                ...
                for _ in range(n_test_decisions):
                    ...
                    if cfg.dim_context > 0:
                        ctx = torch.randn(1, cfg.dim_context, device=cfg.device).clamp_(-3, 3)
    ...
    for x_val in x_values:
        for comp_vals in comp_iter:
            ...
            for seed in range(n_seeds):
                ...
                if x_axis_name in params: params[x_axis_name] = x_val
                ...
                for k, v in params.items():
                    if hasattr(base_cfg, k): cfg_args[k] = v
                
                if hasattr(base_cfg, x_axis_name): cfg_args[x_axis_name] = x_val
...
def plot_metric_scaling(
    ...
    def move_to_cpu(val):
        if isinstance(val, torch.Tensor): return val.detach().cpu()
        return val
    ...
    df_agg = plot_df.groupby([x_col, hue_col] if hue_col else [x_col]).apply(compute_pooled_metric, include_groups=False).reset_index(name=y_label)
    ...
    ax.legend(loc=legend_loc, frameon=True, edgecolor='black', framealpha=0.95, fancybox=False)
...
if __name__ == "__main__":
    ...
    def exp4():
        ...
        plot_metric_scaling(df, 'n_samples', 'N', 'p0_error', 'multi_sim_method', "Multi-Sim Aggregation",log_y=False)
