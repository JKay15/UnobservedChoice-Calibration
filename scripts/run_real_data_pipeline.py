...
def run_pipeline(
    ...
    if mnl_lr is not None:
        cfg.mnl_lr = mnl_lr
    if mnl_epochs is not None:
        cfg.mnl_epochs = mnl_epochs
    ...
    X_train, y_train, _ = train_loader.get_catboost_data(
        target_col="booking_bool", only_clicks=True
    )
    ...
    s_series = current_sales_df.groupby("srch_id")["u_hat"].apply(
        lambda x: logsumexp(x.values)
    )

    ctx_df = current_sales_df.groupby("srch_id")[list(cfg.context_cols)].first()

    # Simulator Preds
    current_sales_df["sim_p_book"] = sim_model.predict_proba(
        current_sales_df[all_feat_cols]
    )[:, 1]
    ...
    print(
        f"   Gamma Norms | Linear: {gamma_lin.norm().item():.2f}, MRC: {gamma_mrc.norm().item():.2f}"
    )
    ...
    y_true = (
        test_df.groupby("srch_id")["booking_bool"]
        .sum()
    ...
    test_s_hat = test_df.groupby("srch_id")["u_hat"].apply(
        lambda x: logsumexp(x.values)
    )
    ...
    print(
        f"   [Diag] P_Lin: Mean={p_lin.mean():.4f}, Min={p_lin.min():.6f}, Max={p_lin.max():.6f}"
    )
    print(
        f"   [Diag] P_MRC: Mean={p_mrc.mean():.4f}, Min={p_mrc.min():.6f}, Max={p_mrc.max():.6f}"
    )
    ...
    if return_preds:
        result.update(
            {"y_true": y_true, "p_sim": sim_p_nobuy, "p_lin": p_lin, "p_mrc": p_mrc}
        )
...
def plot_real_data_results(res_lin, res_nn):
    ...
    colors = [c_sim, lighten(c_lin, 0.4), c_lin, lighten(c_mrc, 0.4), c_mrc]
    ...
    def save_bar_chart(metric_key):
        ...
        bars = ax.bar(
            x_pos, data, color=colors, edgecolor="black", linewidth=1.5, width=0.65
        )
...
if __name__ == "__main__":
    ...
    res_nn = run_pipeline(
        utility_model_type="neural", mnl_lr=0.001, mnl_epochs=50, return_preds=True
    )
